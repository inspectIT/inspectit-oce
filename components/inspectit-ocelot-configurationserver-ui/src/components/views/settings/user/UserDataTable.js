  ...
  render() {
    ...
    let filteredUsers = this.props.users.filter((user) => {
      if(user.head || user.newUser || user.username || user.id){
        return user.head || user.newUser || user.username.includes(userFilter) || user.id.toString().includes(userFilter)
      }
    })
...
function mapStateToProps(state) {
  const {users, pendingRequests, userFilter, error, success} = state.settings
  return {
    users,
    pendingRequests, 
    userFilter,
